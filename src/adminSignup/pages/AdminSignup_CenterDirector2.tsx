...
const AdminSignupStep4 = () => {
  ...
  const [centerGrade, setCenterGrade] = useState<'A' | 'B' | 'C' | 'D' | 'E'>(
    (adminSignupData.centerGrade as 'A' | 'B' | 'C' | 'D' | 'E') || 'A',
  );
  ...
  // 센터 등급 ENUM (CenterGrade)
  const gradeOptions: ('A' | 'B' | 'C' | 'D' | 'E')[] = ['A', 'B', 'C', 'D', 'E'];
  ...
  const handleNext = () => {
    ...
    console.log({ ...adminSignupData, centerDocument, centerGrade, operatingPeriod, description, hasVehicle });
  ...
  return (
    <div className="flex min-h-screen flex-col justify-between px-6 pb-6">
      <div className="mx-auto mt-10 w-full max-w-md">
        ...
        <div className="mb-4 text-left">
          ...
          <div className="flex gap-2">
            ...
            <input
              ...
              onFocus={() => setFocusedInput('operatingPeriod')}
              onBlur={() => setFocusedInput(null)}
              className={`mt-2 w-2/3 rounded-md border px-4 py-3 focus:outline-none ${
                focusedInput === 'operatingPeriod' ? 'border-primary-500' : 'border-gray-300'
              }`}
            />
          </div>
        </div>
        ...
        <div className="mb-4 text-left">
          ...
          <textarea
            ...
            onBlur={() => setFocusedInput(null)}
            className={`mt-2 h-24 w-full rounded-md border px-4 py-3 focus:outline-none ${
      ...
      <div className="mx-auto flex w-full max-w-md gap-2">
        <button
          onClick={() => navigate('/signup/center/step3')}
        ...
        <button
          onClick={handleNext}
          disabled={!centerGrade || !operatingPeriod || !description}
          className={`w-2/3 rounded-md px-4 py-3 font-semibold ${
            !centerGrade || !operatingPeriod || !description
